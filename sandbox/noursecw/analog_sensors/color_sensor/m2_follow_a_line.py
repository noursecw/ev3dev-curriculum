...
def follow_the_line(robot, white_level, black_level):
    ...
    while True:
        ...
        if robot.touch_sensor.is_pressed:
            print('break')
            break
        if abs(robot.color_sensor.reflected_light_intensity - white_level) <= 3:
            # print('turn')
            robot.left_motor.run_forever(speed_sp=speed)
            robot.right_motor.run_forever(speed_sp=speed_turn)
        if abs(robot.color_sensor.reflected_light_intensity - black_level) <= 3:
            # print('drive')
            robot.left_motor.run_forever(speed_sp=speed)
